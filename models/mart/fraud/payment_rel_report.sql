...
--
-- Sources:
--   fct_trips    → trip-grain payment flags and status
--   fct_payments → raw payment events (not deduplicated) for full history
--   dim_drivers  → driver identity and city context
--   dim_riders   → rider identity
--
-- PURPOSE:
--   Payment reliability report for the fraud team.
--   Combines trip-grain payment flags from fct_trips with raw payment
--   event counts from fct_payments to give a complete picture of what
--   happened at each payment attempt — not just the deduplicated outcome.
--   Surfaces trips where payment behaviour is anomalous for investigation.
--
-- Materialisation: table
--   Fraud investigation requires the complete payment history.
--   Incremental would risk missing retrospectively flagged events.
--
-- TAGS: fraud
-- OWNER: fraud team
...
with payment_event_summary as (
    ...
    -- Summarise raw payment events per trip from fct_payments
    -- This gives the full count of attempts, not just the winning payment
...
select
    ...
    t.missing_payment_flag,
    t.duplicate_payment_flag,
    t.invalid_payment_amount_flag,
    ...
    p.total_amount_attempted,
    p.total_fee_attempted,
    p.last_payment_attempt_at,
