...
--
-- Sources:
--   dim_drivers → all driver performance metrics already aggregated
--   dim_cities  → city_name for display
--
-- PURPOSE:
--   Driver leaderboard for the operations team.
--   Ranks drivers within their city across key performance dimensions:
--   trip volume, revenue, completion rate, and reliability.
--   Surfaces underperforming drivers alongside top performers.
--
-- Materialisation: table
--   Rankings must always reflect the complete driver population.
--   Incremental would produce incorrect rank values since rank() is
--   a window function that requires the full dataset to be meaningful.
--
-- TAGS: operations
...
with driver_ranked as (
    ...
    select
        ...
        -- Availability
        d.total_online_hours,
        d.trips_per_online_hour,
        ...
        -- Fraud signals
        d.extreme_surge_trips,
        d.corporate_trips,

        -- Payment reliability
        d.failed_payment_count,
        d.missing_payment_count,
        d.duplicate_payment_count,
        d.invalid_payment_amount_count,
        ...
        case
            when d.completion_rate_pct is not null then
                rank() over (
                    partition by d.city_id
                    order by d.completion_rate_pct desc
                )
        end                                                  as city_completion_rank,
...
select
    ...
    -- Trip volume
    r.driver_lifetime_trips,
    r.cancelled_trips,
    r.no_show_trips,
    r.total_trip_attempts,

    -- Revenue
    r.total_net_revenue_earned,
    r.avg_net_revenue_per_trip,

    -- Availability
    r.total_online_hours,
    r.trips_per_online_hour,
    ...
    -- Payment reliability
    r.failed_payment_count,
    r.missing_payment_count,
    r.duplicate_payment_count,
    r.invalid_payment_amount_count,
    r.total_payment_issues,

    -- City rankings
    r.city_trip_volume_rank,
    r.city_revenue_rank,
    r.city_completion_rank,
    r.city_rating_rank
